...
public class UserLoginController {
    ...
    public ResponseEntity<?> login(@RequestBody UserLoginRequestDTO requestDTO) {
        ...
        boolean isAuthenticated = userLoginService.authenticate(requestDTO.getPosLoginId(), requestDTO.getPosPassword());

        if (isAuthenticated) {
            String token = jwtUtil.generateToken(requestDTO.getPosLoginId()); // ✅ JWT 생성
